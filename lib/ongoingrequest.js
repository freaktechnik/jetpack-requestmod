...
const getStreamFromValue = (value) => {
    var stream, length;
    if(value === null) {
        stream = InputStream("").stream;
        length = 0;
    }
    else if(typeof value === "object" && value instanceof Ci.nsIInputStream) {
        stream = value;
        length = stream.available();
    }
    else {
        stream = InputStream(value).stream;
        length = stream.available();
    }
...
const Listener = Class({
    ...
    onStopRequest: function(request, context, status) {
        ...
        let [ stream, length ] = getStreamFromValue(this.value);
...
const OngoingRequest = Class({
    ...
    set content(val) {
        if(this.direction === OUTGOING) {
            ...
            let [ stream, length ] = getStreamFromValue(val);
            if(val === null) {
                channel.setUploadStream(stream, "", length);
            }
            else {
                // The MIME InputStream already contains all the info headers.
                if(stream instanceof Ci.nsIMIMEInputStream)
